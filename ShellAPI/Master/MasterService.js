...
async function GetChannelMaster() {
  try {
    ...
    let RevenueData = await pool.request().query("EXEC GetChannelList ");
...
async function GetCountryMaster() {
  try {
    ...
    let MasterData = await pool
      .request()
      .query(
        " SELECT cnCountryID , cnCountryName  FROM TBLCOUNTRYMASTER  WHERE cnCountryID IN (1000000120,1000000034,1000000051)  "
      );
    return MasterData.recordsets;
  } catch (error) {
    console.log(error);
...
async function GetCountryWiseStateMaster(CountryId) {
  try {
    ...
    let product = await pool
      ...
      .query(
        "SELECT stStateID, stStateName FROM tblStateMaster WHERE st_cnCountryID= @input_CountryId"
      );
...
async function GetCityWiseStateMaster(StateID) {
  try {
    ...
    let product = await pool
      ...
      .query(
        "SELECT ctCityID, ctCityName FROM tblCityMaster WHERE ct_stStateID= @input_StateID"
      );
    return product.recordsets;
  } catch (error) {
    console.log(error);
...
async function GetlocationMaster() {
  try {
    let pool = await sql.connect(config.configBi);
    let MasterData = await pool.request().query(" EXEC Get_LocationMaster ");
    return MasterData.recordsets;
  } catch (error) {
    console.log(error);
  }
...
async function GetChatBot() {
  try {
    let pool = await sql.connect(config.configBi);
    let MasterData = await pool.request().query(" SELECT id,question 'message',answer FROM questions_answers ");
    return MasterData.recordsets;
  } catch (error) {
    console.log(error);
  }
